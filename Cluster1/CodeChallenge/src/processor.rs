use solana_program::{
    ...
    pubkey::Pubkey,
    sysvar::{rent::Rent, Sysvar, clock::Clock}, clock, 
...
use crate::{error::{EscrowError, self}, instruction::EscrowInstruction, state::Escrow};
...
impl Processor {
    ...
    fn process_init_escrow(
        ...
        let unlock_time = current_slot + 100;
...
fn process_exchange(
    ...
    if clock < escrow_info.unlock_time {
        return Err(EscrowError::EscrowTimeLocked.into());
    }
    if clock >= escrow_info.time_out {
        return Err(EscrowError::Timeout.into());
    }
...
//iterating the pda_account 
    let pda_account = next_account_info(account_info_iter)?;

    //invoking the spl_token program instructions to initiate transfer
...
fn process_cancel(
   ...
   // define the iterator we will pass to iterate through accounts
    let account_info_iter = &mut accounts.iter();
   // get initializers account
    let initializer = next_account_info(account_info_iter)?;
    // we just need to check if initializer is the signer and the tx will fail
    if !initializer.is_signer {
        return Err(ProgramError::MissingRequiredSignature);
    }
...
// escrow info must be mutible to change the values in state
    let mut escrow_info = Escrow::unpack_unchecked(&escrow_account.try_borrow_data()?)?;
    ...
    let current_slot = Clock::get()?.slot;
    escrow_info.unlock_time = current_slot + 100;
